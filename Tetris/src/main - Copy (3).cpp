...
private:
	...
	bool m_Placed = false;
private:
	bool UpdateRows(const std::vector<std::shared_ptr<Piece>>::const_iterator& begin, const std::vector<std::shared_ptr<Piece>>::const_iterator& end) noexcept
	{
		std::unordered_map<float32, uint16> rows;
		bool isRowFull = false;
		
		for (auto it = begin; it != end + 1; ++it) // iterate pieces
		{
			for (size_t i = 0; i < (*it)->m_Rectangles.size(); ++i)
			{
				if((*it)->m_Rectangles[i].Visible())
		...
		for (auto it = rows.begin(); it != rows.end(); ++it)
		{
			if (it->second == 9)
			{
				for (auto itp = begin; itp != end + 1; ++itp) // iterate pieces
				{
					for (size_t i = 0; i < (*itp)->m_Rectangles.size(); ++i)
					{
						if ((*itp)->m_Rectangles[i].GetLUY() == it->first)
		...
		//for (Quad& quad : m_PreviewRectangles)
		{
			...
			if (!QuadHasNeighbour(quad))
			{
				...
				for (std::vector<std::shared_ptr<Piece>>::const_iterator it = begin; it != end; ++it)
				{
					...
					for (const Quad& otherRectangle : (*it)->m_PreviewRectangles)
					{
	...
	bool Move(const float32 x, const float32 y, const std::vector<std::shared_ptr<Piece>>::const_iterator& begin, const std::vector<std::shared_ptr<Piece>>::const_iterator& end, const MoveDir mode) noexcept
	{
		...
		{
			bool canMove = false;
			for (uint8 i = 0; i < m_PreviewRectangles.size(); ++i)
			{
				for(Quad& quad : m_PreviewRectangles)
				{
					Quad temp = quad;
	...
	while (window->pollEvent(event))
	{
		...
		case sf::Event::KeyPressed:
			...
			case sf::Keyboard::Up:
				...
				//currentPieces->at(currentPieces->size() - 1).MoveUp(Piece::PieceSize, currentPieces->cbegin(), currentPieces->cend() - 1);
				break;
			case sf::Keyboard::A:
...
void MovePiecesDown(const std::vector<std::shared_ptr<Piece>>& pieces)
{
	...
	while (size > 0)
	{
		...
		for (size_t i = 0; i < pieces.size(); ++i)
		{
			if(pieces[i]->MoveDown(Piece::PieceSize, pieces.cbegin(), pieces.cend() - 1))
			//if (pieces[i]->MoveReplace(pieces.cbegin(), pieces.cend()) || pieces[i]->MoveDown(Piece::PieceSize, pieces.cbegin(), pieces.cend() - 1))
			{
				++size;
			}
		}
...
int main()
{
    ...
    while (window.isOpen())
    {
		...
		if (currentPieces.at(currentPieces.size() - 1)->Placed())
		{
			...
			if(temp->StateIsValid(currentPieces.cbegin(), currentPieces.cend() - 1))
